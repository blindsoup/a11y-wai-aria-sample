...
function getSubTreeEl(itemEl) {
  const id = itemEl.getAttribute('aria-owns');
...
function getAllTreeEls() {
  ...
  return !!itemEl.hasAttribute('aria-owns');
}
...
function getChildItemEls(treeEl) {
  const liEls = Array.from(treeEl.children);
  return liEls.forEach(function(liEl) {
  debugger;
  	return liEl.firstChild.firstChild;
  });
}
